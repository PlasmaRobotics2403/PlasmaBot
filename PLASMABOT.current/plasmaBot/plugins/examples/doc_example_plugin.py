...
class EXAMPLECOMMAND(PBPlugin):
    ...
    async def on_message(message, message_type, message_context): # plugins have access to all bot events.  Only exception to normall formating is the requirement of message_type and message_context in on_message which supply the sender of the message [owner, self, bot (other bot), or user] as a string or the place where it was sent ([server, direct] as a string for server channels vs direct messages)
