...
class GameGuesses(PlasmaCog):
    """Cog for the FRC Discord 2025 Game Guesses Contest"""
    ...
    @chat_command(name='guess', description='Get Added to the 2025 Game Guesses Channel')
    @only_guild(176186766946992128)
    async def guess(self, ctx):
        """Get Added to the 2025 Game Guesses Channel"""
        await ctx.author.add_roles(ctx.guild.get_role(1322567298283737219))
        await ctx.send('You have been added to the 2025 Game Guesses Channel!', ephemeral=True)
    
    @PlasmaCog.listener()
    async def on_message(self, message):
        """Wait for a message in the 2025 Game Guesses Channel"""
        try:
            if message.channel.id == 1322567875705442334:
