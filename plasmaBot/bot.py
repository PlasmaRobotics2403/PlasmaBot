...
class PlasmaBot(discord.Client):
    ...
    async def cmd_invite(self, message, leftover_args):
        ...
        if self.config.allow_invites or message.author.id == self.config.owner_id:
            if inviteURL == "":
                return Response(
                    'Invite %s to your server!  See: https://discordapp.com/oauth2/authorize?client_id=%s&scope=bot&permissions=0x00200000&permissions=0x00000008' % (self.config.bot_name, self.config.client_id), reply=True, delete_after=30
                )
            else:
                if self.user.bot:
                    return Response(
